...
fun SettingsMessage.toByteArray(): ByteArray {
    return "$grid$DATA_SEPARATION$flashlight$DATA_SEPARATION$stabilization$DATA_SEPARATION$manualSensor$DATA_SEPARATION$equirectangular$DATA_SEPARATION$focus$DATA_SEPARATION$exposure$DATA_SEPARATION$sensitivity$DATA_SEPARATION$antiAliasing$DATA_SEPARATION$imageScaling$ACCUMULATED_DATA_SEPARATION_CHAR".encodeToByteArray()
}
...
fun String.toSettingsMessage(): SettingsMessage {
    ...
    return SettingsMessage(
        ...
        antiAliasing = settingsData[8].toInt(),
        imageScaling = settingsData[9].toInt(),
    )
}